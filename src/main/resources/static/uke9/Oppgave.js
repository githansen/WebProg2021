    ...
    $.get("/uke8/hent", function (data){
        ...
        for(let i of data){
            ut += "<tr><td>" + i.personnr +"</td><td>" + i.navn +"</td><td>" + i.adresse + "</td><td>"
                + i.kjennetegn + "</td><td>" + i.bilmerke + "</td><td>"
                + i.biltype +"</td><td><a class='btn btn-primary' href='endre.html?mnr=" + i.mnr+"'>Endre</a><button class='btn btn-danger' onclick='sletten(" + i.mnr  + ")'>Slett</button></td></tr>"
        }
        ut += "</table>"
        $("#utskrift").html(ut)
    })
